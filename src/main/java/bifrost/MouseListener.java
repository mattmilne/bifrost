...
import static util.Settings.SCREEN_HEIGHT_PIXELS;
import static util.Settings.SCREEN_WIDTH_PIXELS;
...
public class MouseListener {
    ...
    public static void mouseScrollCallback(long window, double xOffset, double yOffset) {
        get().scrollX = xOffset;
        get().scrollY = yOffset;
        System.out.println("Scrolling: (X: " + get().scrollX + ", Y: " + getScrollY() + ")");
    }
    ...
    public static Vector2f getWorld() {
        ...
        Vector4f worldPosition = new Vector4f(currentX, currentY, 0, 1);

        Camera camera = Window.getScene().camera();
        Matrix4f inverseView = new Matrix4f(camera.getInverseView());
        Matrix4f inverseProjection = new Matrix4f(camera.getInverseProjection());
        worldPosition.mul(inverseView.mul(inverseProjection));

        return new Vector2f(worldPosition.x, worldPosition.y);
    }
    ...
    public static Vector2f getScreen() {
        float currentX = getX() - get().gameViewportPos.x;
        currentX = (currentX / get().gameViewportSize.x) * SCREEN_WIDTH_PIXELS;
        float currentY = getY() - get().gameViewportPos.y;
        currentY = SCREEN_HEIGHT_PIXELS - ((currentY / get().gameViewportSize.y) * SCREEN_HEIGHT_PIXELS);
